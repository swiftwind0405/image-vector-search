...
# ---------------------------------------------------------------------------
# bulk_add_category / bulk_remove_category
# ---------------------------------------------------------------------------

class TestBulkAddRemoveCategory:
    def _setup(self, tmp_path):
        repo = _make_repo(tmp_path)
        _insert_image(repo, "h1", "/data/images/img1.jpg")
        _insert_image(repo, "h2", "/data/images/img2.jpg")
    ...
    def test_bulk_add_category_returns_affected_count(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        count = repo.bulk_add_category(["h1", "h2"], cat.id)
        assert count == 2

    def test_bulk_add_category_applies_to_all_hashes(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        repo.bulk_add_category(["h1", "h2", "h3"], cat.id)
        assert len(repo.get_image_categories("h1")) == 1
        assert len(repo.get_image_categories("h2")) == 1
        assert len(repo.get_image_categories("h3")) == 1

    def test_bulk_add_category_ignores_duplicates(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        repo.add_image_to_category("h1", cat.id)
        count = repo.bulk_add_category(["h1", "h2"], cat.id)
        assert count == 1

    def test_bulk_add_category_empty_list_returns_zero(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        count = repo.bulk_add_category([], cat.id)
        assert count == 0

    def test_bulk_remove_category_returns_affected_count(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        repo.add_image_to_category("h1", cat.id)
        repo.add_image_to_category("h2", cat.id)
        count = repo.bulk_remove_category(["h1", "h2"], cat.id)
        assert count == 2

    def test_bulk_remove_category_removes_correct_images(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        repo.add_image_to_category("h1", cat.id)
        repo.add_image_to_category("h2", cat.id)
        repo.add_image_to_category("h3", cat.id)
        repo.bulk_remove_category(["h1", "h2"], cat.id)
        assert repo.get_image_categories("h1") == []
        assert repo.get_image_categories("h2") == []
        assert len(repo.get_image_categories("h3")) == 1

    def test_bulk_remove_category_empty_list_returns_zero(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        count = repo.bulk_remove_category([], cat.id)
        assert count == 0
...
class TestFolderBulkCategory:
    ...
    def _setup(self, tmp_path):
        ...
        _insert_image(repo, "h2", "/data/images/nature/tulip.jpg")
        _insert_image(repo, "h3", "/data/images/city/tower.jpg")
        cat = repo.create_category("Outdoors")
        return repo, cat

    def test_bulk_folder_add_category_adds_to_folder_images(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        count = repo.bulk_folder_add_category("nature", cat.id, self.IMAGES_ROOT)
        assert count == 2
        assert len(repo.get_image_categories("h1")) == 1
        assert len(repo.get_image_categories("h2")) == 1
        assert repo.get_image_categories("h3") == []

    def test_bulk_folder_add_category_returns_zero_for_empty_folder(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        count = repo.bulk_folder_add_category("nonexistent", cat.id, self.IMAGES_ROOT)
        assert count == 0

    def test_bulk_folder_add_category_ignores_duplicates(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        repo.add_image_to_category("h1", cat.id)
        count = repo.bulk_folder_add_category("nature", cat.id, self.IMAGES_ROOT)
        assert count == 1

    def test_bulk_folder_remove_category_removes_from_folder_images(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        repo.add_image_to_category("h1", cat.id)
        repo.add_image_to_category("h2", cat.id)
        repo.add_image_to_category("h3", cat.id)
        count = repo.bulk_folder_remove_category("nature", cat.id, self.IMAGES_ROOT)
        assert count == 2
        assert repo.get_image_categories("h1") == []
        assert repo.get_image_categories("h2") == []
        assert len(repo.get_image_categories("h3")) == 1

    def test_bulk_folder_remove_category_returns_zero_when_not_set(self, tmp_path):
        repo, cat = self._setup(tmp_path)
        count = repo.bulk_folder_remove_category("nature", cat.id, self.IMAGES_ROOT)
        assert count == 0
